...
export default function SubAdminPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 font-sans antialiased selection:bg-lime-400 selection:text-slate-950 p-4 sm:p-6 md:p-8 relative overflow-x-hidden">
      ...
      {/* Active Bookings Classic Scrolling Horizontal Table Layout */}
              ...
              {blockedSlots.map((slot) => {
                ...
                return (
                  <tr key={slot.id} className="hover:bg-white/[0.01] transition-colors">
                    ...
                    <td className="p-4 font-mono text-xs text-slate-300 whitespace-nowrap">{blockDuration} Mins</td>
                    <td className="p-4 font-mono text-xs font-bold text-cyan-400 whitespace-nowrap">{slot.court_number}</td>
                    <td className="p-4 font-mono text-xs text-slate-400 uppercase whitespace-nowrap">{slot.reason}</td>
                  </tr>
                );
              })}
            </tbody>
