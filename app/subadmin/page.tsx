...
/* ------------------------------------------------------------------ */
/*  Motion Presets (mirrors Admin/Coach portal)                       */
/* ------------------------------------------------------------------ */
...
export default function SubAdminPage() {
 ...
 /* ============================================================
    STATE  ▸  identical to original (business logic unchanged)
    ============================================================ */
 ...
 const [availableCourts, setAvailableCourts] = useState<string[]>(["Full Court", "Court 1", "Court 2"]);
 ...
 const filteredBookings = bookings.filter(
   (b) => b.customer_name?.toLowerCase().includes(searchTerm.toLowerCase()) || b.phone?.includes(searchTerm)
 );
 ...
 /* ============================================================
    ...
    ============================================================ */
 const statCards = [
   { label: "Today Slots", value: todaySlots, accent: "text-lime-400", tag: "01", prefix: "" },
   { label: "Tomorrow Slots", value: tomorrowSlots, accent: "text-white", tag: "02", prefix: "" },
   { label: "Today Advance", value: todaysAdvance, accent: "text-emerald-400", tag: "03", prefix: "₹" },
   { label: "Today Balance", value: todaysBalance, accent: "text-red-400", tag: "04", prefix: "₹" },
   { label: "Cash Vault", value: todayCashCollection, accent: "text-white", tag: "05", prefix: "₹" },
   { label: "UPI Nodes", value: todayUpiCollection, accent: "text-white", tag: "06", prefix: "₹" },
 ];
 ...
 return (
   <main className="min-h-screen bg-neutral-950 text-neutral-100 font-sans tracking-tight antialiased relative w-full overflow-x-hidden">
     ...
     <div className="relative z-10 max-w-7xl mx-auto p-4 sm:p-6 md:p-10">
       ...
       >
         <motion.div variants={fadeUp}>
           ...
           <p className="text-neutral-400 text-sm mt-2 font-mono">
             Live desk · Real-time sync · <span className="text-lime-400">{currentMonthLabel}</span>
           </p>
         </motion.div>
       ...
       >
         <div className="w-full md:w-96 relative">
           <input
             ...
             className="w-full p-3.5 bg-neutral-950 text-white border border-neutral-800 focus:border-lime-400 outline-none placeholder:text-neutral-600 text-sm font-medium transition-colors"
           />
         </div>
         <div className="flex flex-wrap items-center gap-3">
           <motion.button
             ...
             className="bg-lime-400 hover:bg-lime-300 text-black font-mono text-xs uppercase tracking-widest px-6 py-3.5 font-black transition-colors"
           >
             ➕ Offline Booking
           </motion.button>
           ...
           <motion.button
             whileHover={{ y: -2 }}
             whileTap={{ scale: 0.97 }}
             className={`font-mono text-xs uppercase tracking-widest px-6 py-3.5 font-black transition-colors ${
               showCoachingPanel
                 ? "bg-lime-400 text-black"
                 : "bg-neutral-900/60 hover:bg-neutral-800 border border-neutral-800 hover:border-lime-400/40 text-neutral-300 hover:text-white"
             }`}
             onClick={() => {
       ...
       <AnimatePresence>
         {showCoachingPanel && (
           <motion.div
             ...
             className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 mt-4 mb-10"
           >
             <motion.section
               variants={fadeUp}
               initial="hidden"
               animate="show"
               className="lg:col-span-1 border border-neutral-900 bg-neutral-900/40 backdrop-blur p-5 sm:p-6 space-y-4 h-fit"
             >
               <LayoutGroup>
                 <div className="grid grid-cols-2 gap-2 p-1.5 border border-neutral-900 bg-neutral-950/60 relative">
                   {[
                     ...
                     <motion.button
                       ...
                       className={`relative py-2 text-[10px] font-mono uppercase tracking-widest transition-colors ${
                         academyTab === t.id ? "text-black font-black" : "text-neutral-500 hover:text-white"
                       }`}
                     >
                       {academyTab === t.id && (
                         <motion.span
                           layoutId="acad-tab-highlight"
               ...
               {academyTab === "new" ? (
                 <form onSubmit={handleAdminEnrollStudent} className="space-y-3">
                   <input
                     ...
                     className="w-full p-3.5 bg-neutral-950 border border-neutral-800 focus:border-lime-400 outline-none text-sm text-white font-medium transition-colors"
                   />
                   <input
                     type="text"
                     ...
                     maxLength={10}
                     className="w-full p-3.5 bg-neutral-950 border border-neutral-800 focus:border-lime-400 outline-none text-sm text-white font-mono font-medium transition-colors"
                   />
                   <div className="space-y-1">
                     <label className="text-[10px] font-mono uppercase tracking-wider text-neutral-400">Date of Birth</label>
                     <input
                       type="date"
                   ...
                   <div className="space-y-1">
                     <label className="text-[10px] font-mono uppercase tracking-wider text-neutral-400">Email ID</label>
                     <input
                       type="email"
                   ...
                   <div className="space-y-1">
                     <label className="text-[10px] font-mono uppercase tracking-wider text-neutral-400">Payment Method</label>
                     <select
                       value={adminNewStudentMethod}
                       onChange={(e) => setAdminNewStudentMethod(e.target.value)}
                       className="w-full p-3.5 bg-neutral-950 border border-neutral-800 focus:border-lime-400 outline-none text-sm text-neutral-200 font-medium transition-colors"
                     >
                       <option value="UPI">UPI</option>
                       <option value="Cash">Cash</option>
                     </select>
                   </div>
                   <div className="p-3 bg-neutral-950 border border-neutral-800 text-xs font-mono font-bold text-lime-400">
                   ...
                   <motion.button
                     ...
                     whileTap={{ scale: 0.97 }}
                     type="submit"
                     className="w-full bg-lime-400 hover:bg-lime-300 text-black font-mono font-black py-3.5 text-xs uppercase tracking-widest transition-colors"
                   >
                     Enroll & Mark Paid
                   </motion.button>
                 </form>
                 ...
                 <form onSubmit={handleAdminOldPayment} className="space-y-3">
                   <div className="space-y-1">
                     <label className="text-[10px] font-mono uppercase tracking-wider text-neutral-400">Select Due Student</label>
                     <select
             ...
             <motion.section
               ...
               initial="hidden"
               animate="show"
               className="lg:col-span-2 border border-neutral-900 bg-neutral-900/30 backdrop-blur overflow-hidden"
             >
               <div className="p-4 sm:p-5 border-b border-neutral-900">
                 <span className="text-[11px] font-mono uppercase tracking-widest text-neutral-500 block">Roster</span>
               ...
               <div className="overflow-x-auto max-h-[420px] overflow-y-auto">
                 <table className="w-full text-left border-collapse min-w-[560px]">
                   <thead>
                     <tr className="border-b border-neutral-900 text-[10px] font-mono uppercase tracking-widest text-neutral-500 bg-neutral-950/40 sticky top-0 z-10">
                         ...
                         return (
                           ...
                           >
                             ...
                             <td className="p-4 space-y-0.5">
                               <div className="font-mono text-neutral-300 text-xs">{s.phone}</div>
                               <div className="text-xs text-neutral-500 truncate max-w-[200px]">{s.email || "-"}</div>
                             </td>
       ...
       {/* ---------- Filter Note ---------- */}
       <p className="mb-3 text-[10px] font-mono uppercase tracking-widest text-neutral-500 px-1">
       ...
       <motion.section
         ...
         className="border border-neutral-900 bg-neutral-900/30 backdrop-blur overflow-hidden mb-10"
       >
         <div className="p-4 sm:p-5 border-b border-neutral-900">
           <span className="text-[11px] font-mono uppercase tracking-widest text-neutral-500 block">Live Feed</span>
           <h2 className="text-base sm:text-lg font-black uppercase text-white mt-0.5">
             ...
             >
               <AnimatePresence>
                 {filteredBookings.map((booking) => {
                   ...
                   let rowColor = "";
                   if (isToday) rowColor = "bg-lime-400/[0.04]";
                   ...
                   return (
                     <motion.tr
                       ...
                       variants={rowItem}
                       layout
                       className={`${rowColor} hover:bg-lime-400/[0.03] transition-colors`}
                     >
                       <td className="p-4">
                         <div className="font-bold text-white">{booking.customer_name}</div>
                         <div className="font-mono text-[11px] text-neutral-500 mt-0.5">{booking.phone}</div>
                       </td>
                         ...
                         <div className="flex items-center gap-2">
                           ...
                           {isTomorrow && (
                             <motion.span
                               initial={{ scale: 0 }}
                               animate={{ scale: 1 }}
                               className="px-2 py-0.5 text-[9px] bg-amber-400/10 border border-amber-400/30 text-amber-400 tracking-wider font-black uppercase"
                             >
                               Tomorrow
                             </motion.span>
                           )}
                         </div>
                         <div className="text-lime-400 mt-1 font-black">
                         ...
                         <div className="mb-1">
                           <span className={`px-2 py-0.5 text-[10px] font-mono uppercase tracking-wider ${
                             booking.booking_type === "Half Court"
                               ? "bg-neutral-500/10 border border-neutral-500/20 text-neutral-300"
                       ...
                       <td className="p-4 font-mono whitespace-nowrap">
                         {booking.balance_amount > 0 ? (
                           ...
                           <span className="px-2 py-1 text-[10px] font-mono uppercase bg-lime-400/10 border border-lime-400/30 text-lime-400 whitespace-nowrap inline-flex items-center gap-1 justify-center">
                             ✅ Paid
                           </span>
                         )}
                       </td>
                       <td className="p-4 text-center whitespace-nowrap">
                         <div className="flex items-center justify-center gap-2">
                           {booking.balance_amount > 0 ? (
                             <motion.button
                               whileHover={{ y: -2, boxShadow: "0 8px 20px rgba(163,230,53,0.35)" }}
                               whileTap={{ scale: 0.95 }}
                               onClick={() => { setSelectedBooking(booking); setShowPaymentModal(true); }}
                               className="bg-lime-400 hover:bg-lime-300 text-black text-xs font-mono uppercase font-black px-4 py-2 transition-colors"
                             >
                               💰 Collect
                             </motion.button>
                           ) : (
                             booking.customer_name !== "Offline Booking" && (
                               <motion.button
                                 ...
                                 className="bg-neutral-900/60 hover:bg-neutral-800 border border-neutral-800 hover:border-amber-500/40 text-amber-400 text-xs font-mono uppercase px-4 py-2 transition-colors"
                               >
                                 🔄 Reset
                               </motion.button>
                             )
       ...
       >
         <div className="p-4 sm:p-5 border-b border-neutral-900">
           <span className="text-[11px] font-mono uppercase tracking-widest text-neutral-500 block">Feed // Read-only</span>
           <h2 className="text-base sm:text-lg font-black uppercase text-white mt-0.5">
             🚫 Admin Field Blocks
           </h2>
         </div>
               ...
               {blockedSlots.map((slot) => {
                 ...
                 return (
                   <motion.tr
                     ...
                     variants={rowItem}
                     whileHover={{ x: 4 }}
                     className="hover:bg-red-500/[0.04] transition-colors"
                   >
                     ...
                     <td className="p-4 font-mono text-xs text-neutral-400 uppercase">{slot.reason}</td>
                   </motion.tr>
                 );
               })}
             </motion.tbody>
     ...
     <AnimatePresence>
       {showManageSlots && (
         <motion.div
           ...
           animate="show"
           exit="exit"
           className="fixed inset-0 bg-neutral-950/85 backdrop-blur-md flex items-center justify-center p-4 z-[9999]"
         >
           <motion.div
             variants={modalCard}
             initial="hidden"
             animate="show"
             exit="exit"
             className="bg-neutral-900/95 border border-neutral-800 p-5 sm:p-6 w-full max-w-md shadow-2xl space-y-4 relative"
           >
             <div className="absolute -top-px left-0 w-16 h-0.5 bg-lime-400" />
             <div>
               <span className="text-[10px] font-mono uppercase tracking-widest text-lime-400">// Offline Registration</span>
               <h2 className="text-xl sm:text-2xl font-black uppercase tracking-tighter text-white mt-1">
                 Register Offline Booking
               </h2>
               <p className="text-neutral-400 text-xs mt-1 font-mono">
                 Log a manual offline booking into the system.
               </p>
             </div>

             <div className="space-y-3.5">
               <div className="space-y-1.5">
                 <label className="block text-[10px] font-mono uppercase tracking-wider text-neutral-400">Target Date</label>
                 <input
                   type="date"
                   min={new Date().toISOString().split("T")[0]}
                   value={slotDate}
                   onChange={(e) => { setSlotDate(e.target.value); loadAvailableAdminSlots(e.target.value); }}
                   className="w-full p-3.5 bg-neutral-950 text-white border border-neutral-800 focus:border-lime-400 outline-none text-sm font-medium transition-colors"
                   style={{ colorScheme: "dark" }}
                 />
               </div>

               <div className="space-y-1.5">
                 <label className="block text-[10px] font-mono uppercase tracking-wider text-neutral-400">Launch Time</label>
                 <select
                   value={slotTime}
                   onChange={(e) => { setSlotTime(e.target.value); if (slotDate) loadAvailableCourts(slotDate, e.target.value); }}
                   className="w-full p-3.5 bg-neutral-950 text-white border border-neutral-800 focus:border-lime-400 outline-none text-sm font-medium transition-colors"
                 >
               ...
               <div className="grid grid-cols-2 gap-3">
                 <div className="space-y-1.5">
                   <label className="block text-[10px] font-mono uppercase tracking-wider text-neutral-400">Duration</label>
                   <select
                     value={slotDuration}
                 ...
                 <div className="space-y-1.5">
                   <label className="block text-[10px] font-mono uppercase tracking-wider text-neutral-400">Court</label>
                   <select
                     value={slotCourt}
               ...
               <div className="p-4 bg-neutral-950 border border-neutral-800 space-y-3">
                 <label className="block text-[10px] font-mono uppercase tracking-wider text-lime-400">Payment Collection</label>
                 ...
                 {offlinePaymentMethod !== "Cash + UPI" && (
                   <input
                     ...
                     className="w-full p-3 bg-neutral-900 text-white border border-neutral-800 focus:border-lime-400 outline-none text-sm font-medium transition-colors"
                   />
                 )}
                 ...
                 {offlinePaymentMethod === "Cash + UPI" && (
                   <div className="grid grid-cols-2 gap-2">
                     <input
                       ...
                       className="w-full p-3 bg-neutral-900 text-white border border-neutral-800 focus:border-lime-400 outline-none text-sm transition-colors"
                     />
                     <input
                       type="number"
             ...
             <div className="grid grid-cols-2 gap-3 pt-2">
               ...
               <motion.button
                 ...
                 className="w-full bg-neutral-900/60 hover:bg-neutral-800 border border-neutral-800 text-neutral-300 font-mono font-bold py-3.5 text-xs uppercase tracking-widest transition-colors"
               >
                 Cancel
               </motion.button>
             </div>
       ...
       {showPaymentModal && (
         ...
         >
           <motion.div
             ...
             animate="show"
             exit="exit"
             className="bg-neutral-900/95 border border-neutral-800 p-5 sm:p-6 w-full max-w-sm shadow-2xl space-y-4 relative"
           >
             <div className="absolute -top-px left-0 w-16 h-0.5 bg-lime-400" />
             <div>
               <span className="text-[10px] font-mono uppercase tracking-widest text-lime-400">// Balance Settlement</span>
               <h2 className="text-xl sm:text-2xl font-black uppercase tracking-tighter text-white mt-1">
                 💰 Balance Clearing
               </h2>
               <p className="text-neutral-400 text-xs mt-1 font-mono">
             ...
             <div className="p-4 bg-neutral-950 border border-neutral-800 flex justify-between items-center">
               <span className="text-[10px] font-mono uppercase tracking-widest text-neutral-400">Outstanding Balance</span>
               <AnimatePresence mode="wait">
                 <motion.span
                   key={selectedBooking?.balance_amount || 0}
             ...
             <div className="space-y-3.5">
               <div className="space-y-1.5">
                 <label className="block text-[10px] font-mono uppercase tracking-wider text-neutral-400">Payment Route</label>
                 <div className="relative">
                   <select
                     value={paymentType}
                     onChange={(e) => setPaymentType(e.target.value)}
                     className="w-full p-3.5 bg-neutral-950 text-white border border-neutral-800 focus:border-lime-400 outline-none appearance-none text-sm font-medium transition-colors"
                   >
               ...
               {paymentType === "Cash + UPI" && (
                 <motion.div
                   ...
                   className="grid grid-cols-2 gap-2 p-3 bg-neutral-950 border border-neutral-800"
                 >
                   <input
                     type="number"
