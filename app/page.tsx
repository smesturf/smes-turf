...
export default function Home() {
  ...
  const [instaHandle, setInstaHandle] = useState(""); // 📸 TRACKING INSTAGRAM HANDLE STATE
  const [sport, setSport] = useState("Football");
  const [bookingDate, setBookingDate] = useState("");
  const [startTime, setStartTime] = useState("");
  
  // ⏳ LIVE FOMO COUNTDOWN ENGINE STATE
  const [countdown, setCountdown] = useState({ days: 0, hours: 0, minutes: 0, seconds: 0 });

  // 🔒 LAUNCH PROMO LOCKS: Fixed parameters mapping directly to your launch strategy
  ...
  useEffect(() => {
    // Campaign finishes 5 days out relative to launch parameters
    const targetTime = new Date("2026-07-16T23:59:59+05:30").getTime();
  ...
  // 🔒 LAUNCH CONSTRAINT: Dynamically calculates the strict 5-day operational deadline threshold
  ...
  const openRazorpay = async () => {
    try {
      ...
      // 🔒 MAIN CONDITION CHECK: Block verification pre-checkout if phone number already holds a record
      const { data: duplicatePromoCheck, error: duplicateCheckError } = await supabase
        .from("bookings")
        .select("id")
        .eq("phone", phone);
  ...
  const handleBooking = async (paymentData?: any) => {
    // 🔒 SAFETIES ENFORCEMENT ON FINAL SUBMISSION
    ...
    const { data: insertedData, error } = await supabase.from("bookings").insert([
      {
        // 🔒 SCHEMA CONSTRAINTS FIX: Appends handle to the customer name string to guarantee execution safety
    ...
    alert("✅ Launch Special Booking Confirmed! Dispatched via WhatsApp.");
    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-100 font-sans tracking-tight antialiased relative w-full overflow-x-hidden">
      ...
      <header className="max-w-7xl mx-auto px-4 pt-16 pb-6 text-center relative z-10 flex flex-col items-center">
        <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-slate-900/80 border border-slate-800 text-xs font-semibold uppercase tracking-widest text-lime-400 mb-6">
          🚀 GRAND OPENING PROMO CELEBRATION
        </div>
        ...
        <div className="mt-6 px-6 py-3 bg-neutral-900/60 border border-neutral-800/80 font-mono text-xs text-white uppercase tracking-widest flex items-center gap-4 shadow-xl">
          <span className="text-amber-400 font-bold animate-pulse">⏰ OFFER EXPIRES IN:</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="space-y-4">
              ...
              {/* Registration Grid Matrix Including Verified Instagram Handle Entry Row */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <label className="text-xs font-mono uppercase text-neutral-400">Full Name</label>
                  <input type="text" placeholder="Athlete name" value={name} onChange={(e) => setName(e.target.value)} className="w-full p-4 bg-neutral-900/50 border border-neutral-800 text-xs font-medium outline-none text-white focus:border-lime-400" />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-mono uppercase text-neutral-400">Phone Number</label>
                  <input type="tel" placeholder="10 Digits contact" value={phone} onChange={(e) => { const s = e.target.value.replace(/\D/g, ""); if (s.length <= 10) setPhone(s); }} className="w-full p-4 bg-neutral-900/50 border border-neutral-800 text-xs font-mono outline-none text-white focus:border-lime-400" />
                </div>
                
                {/* 📸 MANDATORY INSTAGRAM HANDLE REGISTRATION INPUT INPUT BOX */}
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full p-4 bg-neutral-900/50 border border-neutral-800 text-xs font-mono outline-none text-lime-400 font-bold focus:border-lime-400 placeholder-neutral-600" 
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select value={sport} onChange={(e) => setSport(e.target.value)} className="w-full p-4 bg-neutral-900 border border-neutral-800 text-sm font-medium outline-none text-white">
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full p-4 bg-neutral-900 text-white border border-neutral-800 font-medium text-sm outline-none" 
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-end">
                  ...
                  {/* 🔥 DYNAMIC SLOT SCARCITY BADGE ENGINE */}
                  {bookingDate && (
                ...
                <div className={`grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 p-4 bg-neutral-900/30 border border-neutral-800 max-h-[260px] overflow-y-auto ${!bookingDate ? "opacity-30 pointer-events-none" : ""}`}>
                  {allSlots.map((slot) => {
                    const available = isSlotAvailable(slot);
                    const selected = startTime === slot;
                    return (
                      <button key={slot} disabled={!available} onClick={() => setStartTime(slot)} className={`py-3 text-[11px] font-mono font-bold uppercase border ${selected ? "bg-red-600 border-red-500 text-white" : available ? "bg-lime-500/10 border-lime-500/30 text-lime-400 hover:bg-lime-500 hover:text-black" : "bg-neutral-950 border-neutral-900 text-neutral-600 opacity-40 cursor-not-allowed"}`}>
                        {slot}
                      </button>
                    );
                  })}
          ...
          <div className="lg:col-span-5 bg-neutral-900/50 border border-neutral-900 p-6 space-y-6 lg:sticky lg:top-6">
            ...
            {/* 🧾 PITCH BILL RECEIPT: Swapped Matrix matching 20px / 17.7px requirement rules */}
            ...
            {/* 📸 MANDATORY SOCIAL GROWTH HACK VERIFICATION BLOCKBOX */}
            <div className="flex items-start gap-3 p-4 bg-black border border-neutral-800">
              <input 
                ...
                className="w-5 h-5 accent-lime-400 mt-0.5 cursor-pointer flex-shrink-0" 
              />
            ...
            <button 
              disabled={!startTime || !instaAgreed || !instaHandle} 
              onClick={openRazorpay} 
              className={`w-full font-mono text-xs uppercase tracking-widest py-4 font-black transition-all ${!startTime || !instaAgreed || !instaHandle ? "bg-neutral-800 text-neutral-500 cursor-not-allowed" : "bg-lime-400 hover:bg-lime-300 text-black cursor-pointer"}`}
            >
              {!startTime ? "Select Kickoff Slot" : !instaHandle ? "Provide Instagram Handle" : !instaAgreed ? "Accept Terms & Conditions" : "Confirm Launch Booking"}
            </button>
          </div>
