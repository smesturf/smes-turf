I've reviewed your `App.tsx` file for **SMES Turf** — a beautiful sports arena booking app with Supabase + Razorpay + WhatsApp integration. I'll now redesign the UI to be **smoother, more polished, and heavily animated with Framer Motion**, while keeping *all your business logic intact* (bookings, payments, staff login, slot checking, WhatsApp dispatch).

Here's what I'm upgrading:
- **Framer Motion everywhere** — page-load orchestration, scroll-triggered reveals, staggered children, hover micro-interactions, layout animations on the summary panel, and animated modal transitions
- **Smoother visuals** — refined gradients, animated aurora/grid background, glassmorphism on the summary card, softer borders, better spacing rhythm
- **Slot grid** — animated selection state with `layoutId` glow, staggered mount
- **Live receipt** — animated number transitions when total changes
- **Sticky, animated CTA** and improved hamburger/modal
- Same theme (neutral-950 + lime-400) but tightened and more premium

Let me write the final file:
Action: file_editor create /app/App.tsx --file-text "\"use client\";
...
"
Observation: Create successful: /app/App.tsx
